...
3. backup 파일 생성 확인:
   ```
   ls -la "$HOME/Library/Application Support/ClaudeAlarmTerminal/"*.bak* 2>/dev/null
   ```

**기대 결과**

- 한국어 schema 변환 성공 다이얼로그가 **정확히 1회** 표시
- `.bak` (또는 timestamped backup) 파일 **1개** 생성
...
| --- | --- | --- | --- |
| W-3-1 schema migration | _시연 대기_ | — | — |
| W-3-2 좌우 분할 | _시연 대기_ | — | — |
| W-3-3 탭 cascade | _시연 대기_ | — | — |

> **상태**: Day 3 코드 구현 + 자동 테스트(257건 전부 통과) 완료. 위 manual walkthrough 3건은 결정자(장군) 직접 시연 + sign-off 대기 중.
