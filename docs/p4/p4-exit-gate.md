...
| (g) | **앱이 데몬을 기동하지 않음** — 데몬은 dev CLI(`--serve-probe`)·테스트에서만 기동. A8(loopback listen)은 probe로 검증(앱 아님). Scope(in) #1 미완 | `grep WSServer(\|SessionDaemon( Sources/{앱계층}` => 0건 | P5 (앱 시작 시 데몬 기동) |
| (h) | **`.internal` 입력 경로 component-only** — `InternalSink`/`GhosttySurfaceInjector`는 테스트에서만 생성, live 세션 미배선(`attachExternalSession`은 external만). D5 internal 절반은 빌드·mock 검증됐으나 앱에서 구동 불가 | `grep "InternalSink(\|GhosttySurfaceInjector(" Sources/` => 테스트 외 0건; C2 sign-off 선행조건 | P5 (live surface 배선) |
