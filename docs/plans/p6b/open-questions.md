...
- [x] claim 승격 책임 위치 — **데몬 레이어 `DevicePromotionCoordinator` 확정**. UI측 승격 hook은 claim 미경유로 dead path(정상 디바이스 5분 만료 버그). revoke Coordinator와 대칭 설계. `PairingSession.issue(payload:deviceId:)` 시그니처 보강 + `onClaimed` 콜백 + `DeviceStore.find(byDeviceId:)` 추가 동반. UI는 refreshDevices polling으로 stale 흡수
...
- [ ] D-1 옵션 최종 확정 (A: Tailscale IP 바인딩 vs C: `tailscale serve` 프록시) — Day 0 스파이크 결과에 따라 확정. 스파이크가 "100.x 바인딩이 tailnet 한정 노출"을 실증하면 A, 실패 시 C 폴백
