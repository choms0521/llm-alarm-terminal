...
- [x] **(1) 설정 페이지 진입:** 메인 창 좌측 사이드바 하단의 "설정" 버튼을 클릭한다.
      **별도 창이 열리는 것이 아니라** 메인 창 전체가 설정 페이지로 전환된다.
      좌측 nav에 "디바이스 페어링" 항목이 파란 배경으로 기본 선택된다.
      (보조: 앱 메뉴 → "설정…" 또는 Cmd+,로도 동일하게 전환된다.)
- [x] **(2) 코드 발급:** 우측 콘텐츠 영역의 "코드 발급" 버튼을 누른다. QR 코드 이미지가
      렌더되고, 우측에 6자리 코드("123 456" 형태)가 크게 표시된다.
- [x] **(3) 카운트다운 감소:** "N초 후 만료" 텍스트의 N이 1초마다 줄어드는 것을 관측한다
      (30초 이하로 떨어지면 주황색으로 바뀐다).
- [x] **(4) 새 코드 발급:** "새 코드 발급" 버튼을 누르면 QR/6자리 코드/카운트다운이
      새 값으로 갱신된다.
- [x] **(5) 디바이스 목록:** "등록된 디바이스" 카드에 디바이스 행이 표시된다
      (이름 + 토큰 앞 8자 마스킹 + 만료일). secret 평문은 어디에도 표시되지 않는다.
- [x] **(6) secret 미노출 육안 확인:** 설정 페이지 전체에서 base64url secret 평문이
      어떤 라벨/텍스트로도 노출되지 않음을 확인한다(QR 이미지와 6자리 코드만 노출).
- [x] **(7) 푸시 알림 전환:** 좌측 nav의 "푸시 알림" 항목을 클릭하면 우측 콘텐츠가
      "연결 중일 때 푸시 건너뛰기" 토글 카드로 전환된다.
- [x] **(8) 돌아가기:** 좌측 nav 하단의 "← 돌아가기" 버튼을 클릭하면 메인 화면(HSplitView)
      으로 복귀되고 워크스페이스가 정상 표시된다.

### 친람 결과

2026-06-12 사용자 육안 확인 완료. 위 8개 항목 전부 통과.

친람 과정에서 발견·수정된 사항:

- 설정 페이지 전환이 클릭 직후 재렌더되지 않는 결함 — `AppSettingsState`를 관찰하는
  뷰가 SwiftUI 계층에 없어 수동 `Binding`만으로는 갱신이 전파되지 않았다.
  `SettingsObservingHost`(`@ObservedObject`) 래퍼로 수정 (`03a6f1f`).
- 사용자 요청으로 등록 디바이스 삭제 기능 추가 — 디바이스 행 휴지통 버튼 + 파괴적
  확인 다이얼로그. `DeviceStore.remove(id:)`가 Keychain item과 secret을 영구 삭제하여
  해당 토큰의 WS 인증이 즉시 거부됨을 테스트로 실증 (`56357d1`).

최종 테스트 기준선: SessionTests 183 + WorkspaceTests 83 + DaemonTests 117 = **383 green**.
