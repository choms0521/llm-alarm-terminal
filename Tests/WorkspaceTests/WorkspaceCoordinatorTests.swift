...
final class WorkspaceCoordinatorTests: XCTestCase {
    ...
    func test_addPane_attachesSessionId_toPane() async throws {
        ...
        // default shell pane 이 attach 된 상태로 반환.
        guard let topPane = ws.panes.first else {
            XCTFail("default pane 부재"); return
        }
        XCTAssertNotNil(topPane.sessionId, "default pane 에 session id 부착")

        // 추가 pane.
        let added = await coordinator.addPane(workspaceId: ws.id, kind: .claude)
        XCTAssertNotNil(added?.sessionId, "추가 pane 에 session id 부착")
    }
    ...
    func test_addWorkspace_attachesSessionForDefaultPane() async throws {
        ...
        XCTAssertNotNil(pane?.sessionId)
    ...
    func test_closePane_terminatesSession_andRemovesPaneFromWorkspace() async throws {
        ...
        guard let pane = ws.panes.first, let sessionId = pane.sessionId else {
            XCTFail("default pane / sessionId 부재"); return
        }
        // 두 번째 pane 추가.
        ...
        // 첫 pane(.top) 제거 후 남은 pane 이 .top 으로 승격.
