...
/// 3. surfaceUserdata 포인터 → paneId → sessionId 역인덱스 lookup
///
...
public final class SessionActionRouter {
    ...
    private let resolvePaneId: (UnsafeMutableRawPointer?) -> UUID?
    private let resolveSessionId: (UUID) -> UUID?

    public init(
        observer: SessionStatusObserver,
        resolvePaneId: @escaping (UnsafeMutableRawPointer?) -> UUID?,
        resolveSessionId: @escaping (UUID) -> UUID?
    ) {
        self.observer = observer
        self.resolvePaneId = resolvePaneId
        self.resolveSessionId = resolveSessionId
    }

    /// action_cb hop 본문에서 호출. surfaceUserdata 가 nil 이거나 paneId/sessionId
    ...
    public func dispatch(
        ...
        guard let paneId = resolvePaneId(surfaceUserdata),
              let sessionId = resolveSessionId(paneId) else {
            return
        }
